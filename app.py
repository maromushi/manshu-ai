...
if st.button("計算"):
    ...
    def run_ai(order):
        ...
        # ★ ST無効化フラグ（事前計算）
        # ===============================
        st_gap = max(Start) - min(Start)
        
        outer_weak = max(CPI[3:6]) < CPI[0]
        ...
        if TrueNoAttack:
            ...
            # ★ 2差し（条件付き）
            # ===============================
            if CPI[1] >= CPI[0] - 0.08 and Start[1] >= Start[0] - 0.02:
                FS_mult[1] *= 1.15
        ...
        if (
            ...
            # 3が攻め役のときだけ
            if len(attackers) > 0 and attackers[0] == 2:
        
                # インが完全に死んでない
                if (
                    InsideSurvival[0] >= 0.45
                    and Start[0] >= Start[2] - 0.06
                ):
                    SecondAdj[0] *= 1.18   # ←メイン
                    ThirdAdj[0]  *= 1.10
        
                # A級ならさらに残す
                if CLS[0] in ["A1","A2"] and CPI[0] >= 0.48:
                    SecondAdj[0] *= 1.05
    ...
    def run_zure_ai(order):
        ...
        results, ChaosScore, P1, DAS, IS, debug, Start, Mode = run_ai(order)
